<template>
    <div style="padding: 20px; background: #f5f7fa; min-height: 100vh;">
        ...
        <div style="margin-bottom: 30px;">
            ...
            <p style="color: #666; margin: 5px 0 0 0;">Quản lý thông tin, số CCCD và trạng thái hoạt động của khách hàng.</p>
        </div>

        <!-- Search & Filter Bar -->
        <div style="background: white; border-radius: 12px; padding: 20px; margin-bottom: 25px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
            <div style="display: grid; grid-template-columns: 1fr 1fr 150px 150px 150px; gap: 15px; align-items: flex-end;">
                <div style="position: relative;">
                    <input v-model="searchQuery" type="text" placeholder="Tìm tên, email hoặc CCCD..." @input="filterData"
                        style="width: 100%; padding: 12px 15px; border: 2px solid #e2e8f0; border-radius: 8px;">
                </div>

                <select v-model="statusFilter" @change="filterData" style="padding: 12px 15px; border: 2px solid #e2e8f0; border-radius: 8px; cursor: pointer;">
                    <option value="">Tất cả Kích hoạt</option>
                    <option value="1">Đã Kích Hoạt</option>
                    <option value="0">Chưa Kích Hoạt</option>
                </select>

                <select v-model="blockFilter" @change="filterData" style="padding: 12px 15px; border: 2px solid #e2e8f0; border-radius: 8px; cursor: pointer;">
                    <option value="">Tất cả Trạng thái</option>
                    <option value="0">Đang Hoạt Động</option>
                    <option value="1">Bị Khóa</option>
                </select>

                <button @click="loadCustomers" style="padding: 12px; background: #edf2f7; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
        ...
        <div style="background: white; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); overflow: hidden;">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    <tr v-for="customer in filteredCustomers" :key="customer.id" style="border-bottom: 1px solid #e2e8f0;">
                        ...
                        <td style="padding: 15px; text-align: center; font-weight: 500;">{{ customer.cccd || '---' }}</td>
                        ...
                        <td style="padding: 15px; text-align: center;">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                <button @click="viewCustomer(customer)" class="action-btn btn-view"><i class="fa-solid fa-eye"></i></button>
                                <button @click="editCustomer(customer)" class="action-btn btn-edit"><i class="fa-solid fa-pen"></i></button>
                                <button @click="deleteCustomer(customer)" class="action-btn btn-delete"><i class="fa-solid fa-trash"></i></button>
                            </div>
                        </td>
                    </tr>
                </tbody>
            ...
            <div class="modal-content" style="max-width: 500px;">
                ...
                <div class="modal-body" v-if="selectedCustomer">
                    <div style="display: flex; flex-direction: column; gap: 15px; padding: 10px;">
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px dashed #eee; padding-bottom: 8px;">
                        ...
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px dashed #eee; padding-bottom: 8px;">
                            ...
                            <span style="font-weight: 600;">{{ selectedCustomer.so_dien_thoai || 'Chưa cập nhật' }}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px dashed #eee; padding-bottom: 8px;">
                        ...
                        <div style="display: flex; justify-content: space-between; border-bottom: 1px dashed #eee; padding-bottom: 8px;">
                            ...
                            <span :class="selectedCustomer.is_block == 0 ? 'text-success' : 'text-danger'" style="font-weight: 700;">
                ...
                <div class="modal-body" v-if="editingCustomer">
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select v-model="editingCustomer.is_active" style="padding: 10px; border-radius: 6px; border: 1px solid #ddd;">
                                <option value="1">Đã Kích Hoạt</option>
                                <option value="0">Chưa Kích Hoạt</option>
                            </select>
                        </div>
...
.btn-primary-gradient { background: linear-gradient(135deg, #667eea, #764ba2); color: white; border: none; padding: 12px 20px; border-radius: 8px; font-weight: 600; cursor: pointer; }
.modal-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000; }
.modal-content { background: white; border-radius: 12px; width: 90%; max-width: 650px; padding: 25px; }
...
.form-group input { padding: 10px; border: 2px solid #e2e8f0; border-radius: 8px; outline: none; }
.modal-footer { display: flex; gap: 12px; justify-content: flex-end; margin-top: 25px; }
.btn-save { background: #667eea; color: white; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: 600; }
.btn-cancel { background: #f0f0f0; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; }
.badge-success { background: #dcfce7; color: #16a34a; padding: 5px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 700; }
.badge-danger { background: #fee2e2; color: #dc2626; padding: 5px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 700; }
.badge-info { background: #e0f2fe; color: #0369a1; padding: 5px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 700; }
.action-btn { width: 35px; height: 35px; border-radius: 6px; border: none; cursor: pointer; }
