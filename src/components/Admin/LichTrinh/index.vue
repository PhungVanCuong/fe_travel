<template>
    <div style="padding: 20px; background: #f5f7fa; min-height: 100vh;">
        ...
        <div v-else>
            ...
            <div v-for="tour in filteredSchedules" :key="tour.id_tour" style="background: white; border-radius: 12px; padding: 20px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid #e2e8f0;">
                    <div>
                        <h2 style="margin: 0; color: #333; font-size: 1.2rem; font-weight: 700;">{{ tour.ten_tour }}</h2>
                        <p style="margin: 5px 0 0 0; color: #999; font-size: 0.85rem;">Mã Tour: {{ tour.id_tour }} • {{ tour.danh_sach_lich_trinh.length }} hoạt động</p>
                    </div>
                ...
                <div style="overflow-x: auto;">
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tbody>
                            ...
                            <tr v-for="lich_trinh in tour.danh_sach_lich_trinh" :key="lich_trinh.id" style="border-bottom: 1px solid #e2e8f0; transition: background 0.2s;"
                                ...
                                <td style="padding: 12px 15px; text-align: center;">
                                    <div style="display: flex; gap: 8px; justify-content: center;">
                                        <button @click="editSchedule(lich_trinh, tour.id_tour)" class="action-btn btn-edit" title="Chỉnh sửa">
                                            <i class="fa-solid fa-pen-nib"></i>
                                        </button>
                                        <button @click="deleteSchedule(lich_trinh.id)" class="action-btn btn-delete" title="Xóa">
        ...
        <!-- Add/Edit Modal -->
        <div v-if="showFormModal" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000;">
            <div style="background: white; border-radius: 12px; padding: 30px; max-width: 600px; width: 90%; max-height: 90vh; overflow-y: auto; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
                ...
                <form @submit.prevent="saveSchedule" style="color: #333;">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <select v-model="formData.id_tour" 
                            :disabled="isEditing"
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <textarea v-model="formData.tieu_de_hoat_dong" 
                            placeholder="Nhập mô tả hoạt động..."
                            rows="4"
                            maxlength="500"
                            style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-family: inherit; resize: vertical;"
                            required></textarea>
                        <p style="margin: 5px 0 0 0; color: #999; font-size: 0.85rem;">{{ formData.tieu_de_hoat_dong.length }}/500 ký tự</p>
                    </div>
...
.action-btn { width: 36px; height: 36px; border: none; border-radius: 6px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 0.95rem; transition: all 0.2s; }
.btn-edit { background: #fef3c7; color: #d97706; }
.btn-edit:hover { background: #fcd34d; transform: scale(1.05); }
